...
public class NuclearWarServer {
	...
	public static void openServer(int code) throws IOException {
		if (code > 3000 && code < 10000) {
			...
			randomSeed = new Random().nextInt(9999999) + 1;
			...
			try {
				...
				while (interfaces.hasMoreElements()) {
					...
					// filters out 127.0.0.1 and inactive network interfaces
					...
					while (addresses.hasMoreElements()) {
						...
						if (iface.getDisplayName().toLowerCase().contains("wi-fi")
								|| iface.getDisplayName().toLowerCase().contains("wireless")
								|| iface.getDisplayName().toLowerCase().contains("wifi")) {
			...
			server.addListener(new Listener() {
				...
				public void received(Connection conn, Object req) {
					if (server == null)
						return;
					if (!(req instanceof FrameworkMessage.KeepAlive))
					...
					if (req instanceof TurnPacket) {
						...
						if (((StringPacket) req).data.equals("updateNames")) {
							System.out.println("CONECTION " + conn.getRemoteAddressTCP()
									+ " REQUESTED AN UPDATED LIST OF PLAYERS");
							...
							if (req.toString().substring(1, 6).contains("ready")) {
								System.out.println(
										"SERVER RECEIVED UPDATED REQUEST ON WHETHER OR NOT PLAYER IS READY TO PLAY");
								server.sendToAllTCP(req);
							}
						...
						System.out.println("Connection " + conn.getID() + " sent keepalive request");
					}
				}
				...
				public void connected(Connection conn) {
					...
					server.sendToTCP(conn.getID(), new StringPacket("#genDeckSeed%" + randomSeed));
					...
					if (server.getConnections().size() != ((MultiplayerQueueStage) Launcher.mq_stage).player_names
						...
						System.out.println("SENDING OUT UPDATE PACKET\nNUM CONNECTIONS: "
								+ server.getConnections().size() + "\nNUM SUPPOSED PLAYERS: "
								+ ((MultiplayerQueueStage) Launcher.mq_stage).player_names.size());
		...
		if (code > 3000 && code < 10000) {
			...
			client.addListener(new Listener() {
				...
				public void connected(Connection conn) {
					...
					StringPacket pdp = new StringPacket(
							(String) Backendless.UserService.CurrentUser().getProperty("name"));
					client.sendTCP(pdp);
				...
				public void received(Connection conn, Object dat) {
					if (client == null)
						return;
					client.sendTCP(FrameworkMessage.keepAlive); // makes sure the server keeps the connection alive
																// while the connection is sending any data packet
					if (dat instanceof TurnPacket) {
						...
						if (((MainGameStage) Launcher.game_stage).players.containsKey(data.getIssuer())) {
							...
							if (!data.getIssuer().equals(((MainGameStage) Launcher.game_stage).clientPlayer.username)) {
//								((MainGameStage) Launcher.game_stage).players.get(data.getIssuer()).placemat
//										.advance(((MainGameStage) Launcher.game_stage), data.getIssuer());
							}
						} else {
							System.err.println("Invalid username, " + data.getIssuer() + "possible hacking attempt");
						}
...
//							((MainGameStage)Launcher.game_stage).selectedDelivery = selectedDelivery;
						} else if (data.toString().contains("#genDeckSeed%")) {
							randomSeed = Integer.parseInt(data.toString().split("%")[1]);
							((MultiplayerQueueStage) Launcher.mq_stage).shouldLoad = true;
						} else {
	...
	public static void disconnectClient() {
		...
		StringPacket removePlayer = new StringPacket(
